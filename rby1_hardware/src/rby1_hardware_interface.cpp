...
hardware_interface::return_type RBY1HardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&) 
{
  ...
  wheel_acceleration_limit << 50, 50;
  wheel_velocity << right_wheel(0, 0), left_wheel(0, 0);
  linear_velocity << right_wheel(0, 0), right_wheel(0, 0);
  angular_velocity = left_wheel(0, 0);
  ...
  if (stop_check && right_wheel(0, 0) == 0.0 && left_wheel(0, 0) == 0.0) {
    stop_check = false;
  }
